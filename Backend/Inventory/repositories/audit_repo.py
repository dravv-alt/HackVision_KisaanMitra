...
class AuditRepo:
    ...
    def log(self, farmer_id: str, action: str, meta: Dict[str, Any]) -> None:
        """
        Log an audit entry
        In production, this would write to MongoDB audit_logs collection
        For hackathon, we store in memory and optionally print
        """
        log_entry = {
            "farmerId": farmer_id,
            "action": action,
        ...
        # print(f"[AUDIT] {farmer_id} - {action}: {meta}")
